...
| 4 | 2026-06-01 | L3 | plan | [4-plan-ocr-preprocessing-optimization.md](4-plan-ocr-preprocessing-optimization.md) | 已完成 | 已完成 OCR 图片入口识别、预处理增强、失败诊断、OLE 嵌入图处理、布局/颜色/纹理区域补充、颜色背景局部 det、低对比二值化、低对比 mask 拆行拆列、大框强制拆分、候选排序、视觉分隔合并保护、近重复投票仲裁、全局候选池、分层背景区域识别、局部多预处理/多尺度 det、文本 golden 规则、CTC margin、CTC prefix beam、视觉版面边界、panel 分桶、轻量 deskew、自适应局部预算、模型候选选择、行级 margin 仲裁、超大框优先拆分、宽行滑窗、颜色组件候选、低质 ASCII 噪声过滤、行级修复、高分辨率 tile 补充和本地预算、二维区域候选、det 轮廓投影、trace margin、文本行图、glyph textness、候选评分校准、OCR trace 指标规则、主 det 图像上下文版面、候选 trace 事件、trace_candidate_count、空结果视觉补扫、dominant 背景软前景、保守 det 合并、长行动态宽度/分段、CTC 概率校准、候选池行级 support 仲裁、递归 panel-first 区域识别、低阈值局部 det、多前景 mask 融合、字符级 trace 指标和 trace 指标/metadata 回归入口。 |
...
| 9 | 2026-06-05 | L2 | task | [9-task-ocr-stage-latency-optimization.md](9-task-ocr-stage-latency-optimization.md) | 已完成 | 一次性收紧 `page-regions` / `color-region-det`、对大框改为 direct-first、限制超宽框 split 成本并下调 ORT 默认线程。 |
