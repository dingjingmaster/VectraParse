...
g++ examples/c/smoke.c -Iinclude -Ltarget/release -lvectraparse_ffi -Wl,-rpath,'$ORIGIN/../target/release' -o target/smoke-c
LD_LIBRARY_PATH=target/release ./target/smoke-c >/dev/null
g++ examples/c/smoke.c -Iinclude target/release/libvectraparse_ffi_full.a -ldl -lpthread -lm -o target/smoke-static
./target/smoke-static >/dev/null
...
nm -g target/release/libvectraparse_ffi_full.a | rg "OrtGetApiBase|vectraparse_(detect|parse|result_free|version)" >/dev/null
...
- bundled static archive build/link: pass
